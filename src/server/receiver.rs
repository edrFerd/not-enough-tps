...
pub async fn main() -> anyhow::Result<()> { 
    ...
    let mut mqttoptions = MqttOptions::new(cfg.receiver_id.clone(), cfg.broke_server_ip.clone(), cfg.broke_server_port);
        ...
        match notification {
            Event::Incoming(rumqttc::Packet::Publish(p)) => {
                ...
                if counter % 10000 == 0 {
                    ...
                    println!(
                        "已接收 {} 条 | 总TPS: {total_tps:.3} | 近10k TPS: {interval_tps:.3} | 耗时: {interval_secs:.3}s",
                        counter
                    );
                }
            }
